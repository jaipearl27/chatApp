...
const UserList = ({ chatsList, userName, joinRoom, senderName }) => {
  ...
  useEffect(() => {
    ...
    const getData = setTimeout(() => {
      axios
        .get(
          `${import.meta.env.VITE_API_URL}/employees/employee/${searchInput}`
        )
        .then((res) => {
          if (res?.data?.status) {
            let filteredUsers = res?.data?.employee?.filter(
  ...
  return (
    ...
    >
                ...
                {searchedUsers?.length > 0 ? (
                  searchedUsers?.map((user) => (
                    <div
                      ...
                      onClick={() => {
                        joinRoomHandler(user?.userName, "oneToOne", [
                    ...
                    >
                      ...
                      <IoIosArrowForward
                        className="absolute right-1 text-[#121212]"
                        size={20}
                      />
                    </div>
                  ))
      ...
      <div className="max-h-[90%] overflow-y-auto"></div>
      {chatsList?.map((chat) => (
        <div
          key={chat?._id}
          className="flex flex-row gap-2 px-4 py-2 cursor-pointer hover:bg-slate-100 transition duration-300"
          onClick={() => {
            if (chat?.roomType === "oneToOne") {
              const receiverName = chat?.users?.filter((e) => e !== userName);
              joinRoomHandler(receiverName[0], chat?.roomType, chat?.roomTitle);
              console.log(receiverName[0], chat?.roomType, chat?.roomTitle);
            }
          }}
        >
          ...
          <div className="text-[#121212] flex flex-col justify-center">
            <div className="font-medium text-[20px]">
              {chat?.roomType === "oneToOne" &&
                chat?.roomTitle?.filter((e) => e !== senderName)}
            </div>
